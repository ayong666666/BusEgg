...
功能名称：官网合作表单校验和网页自定义弹框
完成时间：2026-06-12
确认状态：已本地验证并提交 GitHub，暂不部署服务器

涉及前端：
- `web/src/components/ContactForm.vue`
- `web/src/styles.css`

功能说明：
- 姓名必填，长度限制为 2 到 20 个字符
- 联系电话必填，校验中国大陆手机号格式
- 合作类型必选
- 留言非必填，最多 200 字
- 校验不通过时使用网页内自定义弹框提示
- 关闭弹框后自动聚焦到对应输入项
- 提交成功和提交失败也使用同一套自定义弹框

验证方式：
- `web` 执行 `npm run build` 构建通过

部署状态：
- 本次只提交 GitHub
- 等用户明确要求“部署服务器”后再部署到腾讯云
